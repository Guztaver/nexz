...
public class RpgCharacterController {
    ...
    public ResponseEntity<String> deleteUserById(@PathVariable("id") int id) {
        ...
        if (character.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Personagem com ID " + id + " não encontrado. Não foi possível deletar.");
        }
    }

    @PutMapping("/{id}")
    public ResponseEntity<String> updateUserById(@PathVariable("id") int id, @RequestParam String name, @RequestParam int age) {
        Optional<RpgCharacter> optionalCharacter = rpgCharacterRepository.findById(id);

        if (optionalCharacter.isPresent()) {
            RpgCharacter character = optionalCharacter.get(); character.setName(name); character.setAge(age); rpgCharacterRepository.save(character);

            return ResponseEntity.ok("UPDATED");
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Personagem com ID " + id + " não encontrado. Não foi possível atualizar.");
        }
    }
